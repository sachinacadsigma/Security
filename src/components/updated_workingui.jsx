...
const updated_workingui = () => {
  ...
  const [thumbsDownClickedState, setThumbsDownClickedState] = useState(Array(translatedFiles.length).fill(false));
      ...
      if (inputText) {
          ...
          try {
              ...
              if (response.ok) {
                  ...
                  console.error("Error in response:", response.status, response.statusText, errorResponseText);
              }
          } catch (error) {
  ...
  const handleFileChange = (event) => {
    ...
    const unsupportedFiles = files.filter((file) => {
      ...
      return !["pdf", "ppt", "pptx", "xls", "xlsx", "csv", "doc", "docx"].includes(extension);
    });
...
const submitFeedbackDoc = () => {
    ...
    <div className="flex items-center justify-center">
      ...
      <div className="flex items-center justify-center p-2 mb-4 mt-4">
          ...
          <label className="relative inline-flex items-center cursor-pointer">
            ...
            <div className={`dot absolute w-6 h-6 border-2 border-slate-900   bg-white rounded-full shadow transition-transform ${toggle ? 'translate-x-7' : 'translate-x-0'}`}></div> {/* Increased size */}
          </label>
        ...
        />
          ...
          <div className="absolute bottom-12 right-0 mb-4 mr-2">
            {fileErrors.attachFiles && (
              <div className="text-red-500 text-l">{fileErrors.attachFiles}</div>
            )}
        ...
        <div className="flex justify-between items-center mb-4">
          <button
            ...
            className={`px-4 py-2 rounded-md text-white ${
              inputText || translatedText || attachedFiles.length > 0 || 
              fileErrors.attachFiles || fileErrors.attachGlossary 
  ...
  <button
    ...
    className={`text-black ${thumbsUpClickedText ? "text-blue-500" : ""}`}
  >
    <TiThumbsUp size={30} />
  </button>
  <button
    onClick={() => setIsModalOpen(true)} // Open modal for feedback
    className={`text-black ${thumbsDownClickedText ? "text-red-500" : ""}`}
  >
      ...
      {thumbsDownClickedText && (
        <div className="mt-4 border p-2 rounded">
          ...
          >
            ...
            <option value="Translation was incorrect">Translation was incorrect</option>
          ...
          {feedbackOptionText === "Other" && (
            <div>
              ...
              />
              <button
               onClick={() => submitFeedback(feedbackOptionText)}
    ...
    {isModalOpen && (
      <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50">
        <div className="bg-white p-4 rounded shadow-lg mt-20 border border-gray-300">
          ...
          <button onClick={() => setIsModalOpen(false)} className="mt-4 ml-2 mr-2 text-red-500">X</button>
        </div>
      </div>
    )}
          ...
          <table className="min-w-full border-collapse border border-gray-300 mt-4">
            ...
            <tbody>
              {translatedFiles.map((file, index) => (
                <tr key={index}>
                  <td className="border border-gray-300 p-2">{file.source}</td>
                      ...
                      <div className="flex justify-between items-center mb-4">
                        <div className="flex items-center space-x-2">
                          ...
                          <button
                            ...
                            className={`text-black ${thumbsUpClicked ? "text-blue-500" : ""}`}
                          >
                            <TiThumbsUp  size={30} />
                          </button>
                          <button
                            onClick={() => openFeedbackModal(file.source, index)}
                            className={`text-black ${thumbsDownClickedState[index] ? "text-red-500" : ""}`}
                          >
                            <TiThumbsDown size={30} />
                          </button>
                        </div>
          ...
          {isModalOpen && (
            <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50">
              <div className="bg-white border border-gray-400 p-4 rounded shadow-lg mt-20">
                <h3>
                  Feedback for <span className="text-green-800 font-bold">{selectedFileName}</span>
                </h3>
                <select
                ...
                {feedbackSubmitted && (
                  <span className="text-green-500 mt-2 block">Feedback submitted!</span>
                )}
                <button onClick={closeModal} className="mt-4 mr-2 ml-2 text-red-500"> <FaTimes/> </button>
              </div>
            </div>
          )}
        </>
...
export default updated_workingui;
  ...
  const submitFeedback = async (feedbackText) => { 
    ...
    try {
      const response = await axios.post(
        "https://testingfunctionservice.azurewebsites.net/api/feedback_store?code=Xcpu9fp9MR5kdBPqvPKRuWMi4CQdzYhmcDP8cCud6rcxAzFuf_XA9g%3D%3D",
        feedbackData,
        {
          headers: {
      ...
      setTimeout(() => {
        ...
        setTimeout(() => setShowThankYouMessage(false), 1300);
      }, 300);
